...
public class Mountains implements ISoundType<BiomeSound> {
    ...
    public static final Predicate<Holder<Biome>> VALID_BIOME = holder -> holder.is(BiomeTags.IS_MOUNTAIN)
        || holder.is(BiomeTagHelperProxy.getMountain());

    public Mountains() {
        DAY_SOUND = SoundEvent.createVariableRangeEvent(Charmonium.makeId("biome.mountains.day"));
        NIGHT_SOUND = SoundEvent.createVariableRangeEvent(Charmonium.makeId("biome.mountains.night"));
    }
