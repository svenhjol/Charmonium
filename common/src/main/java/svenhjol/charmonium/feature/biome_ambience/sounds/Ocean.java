...
public class Ocean implements ISoundType<BiomeSound> {
    ...
    public static final Predicate<Holder<Biome>> VALID_BIOME =
        holder -> holder.is(BiomeTags.IS_OCEAN) || holder.is(BiomeTagHelperProxy.getOcean());

    public Ocean() {
        DAY_SOUND = SoundEvent.createVariableRangeEvent(Charmonium.makeId("biome.ocean.day"));
        NIGHT_SOUND = SoundEvent.createVariableRangeEvent(Charmonium.makeId("biome.ocean.night"));
    }
